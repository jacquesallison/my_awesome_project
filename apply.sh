...
original_author="jacquesallison"
original_name="my_awesome_project"
original_urlname="my_awesome_project"
original_description="Awesome my_awesome_project created by jacquesallison" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/my_awesome_project "${name}"
...
then
    ...
    echo "then run 'my_awesome_project' or 'python -m my_awesome_project'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'my_awesome_project' or 'python -m my_awesome_project' or 'poetry run my_awesome_project'"
fi
